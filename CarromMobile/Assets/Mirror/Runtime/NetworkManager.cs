...
namespace Mirror
{
    ...
    public class NetworkManager : MonoBehaviour
    {
        ...
        public GameObject pluginContoller;
        ...
        public virtual void Awake()
        {
            ...
            pluginContoller = GameObject.FindWithTag("PluginController");
            slider = pluginContoller.transform.Find("Canvas/Panel_LoadinScreen/Slider").gameObject.GetComponent<Slider>();
        ...
        public void StopHost()
        {
           ...
           // StopClientByServer("Host is closing/leaving the Lobby/Game");            //  My Modification display message when host close or leave the lobby
            StopClient();
            StopServer();
        }
        ...
        public void StopServer()
        {
            ...
            if (!string.IsNullOrEmpty(offlineScene) && !IsSceneActive(offlineScene))
            {
                Debug.Log("Server Changing scen");
       ...
       /* public void StopClientByServer(string message)
        {
            if (authenticator != null)
                authenticator.OnClientAuthenticated.RemoveListener(OnClientAuthenticated);

            OnStopClient(message);
        ...
        public virtual void ServerChangeScene(string newSceneName)
        {
            ...
            // loadingSceneAsync = SceneManager.LoadSceneAsync(newSceneName);   //inetial statement
            StartCoroutine(LoadAsynchronously(newSceneName));             //    my Modification
            if (newSceneName.StartsWith("Assets/Scenes/Lobby"))           //    to display
            {                                                             //    loading screen and bar
                OnSceneReload();  //this method is my modification,look bellow              //to destroy current plugin controller and netwrok manager
                                          // 
            }                                                             //
        ...
        internal void ClientChangeScene(string newSceneName, SceneOperation sceneOperation = SceneOperation.Normal, bool customHandling = false)
        {
            ...
            switch (sceneOperation)
            {
                case SceneOperation.Normal:
                    {
                        ///        My Modification to display loading screen in scene change
                        StartCoroutine(LoadAsynchronously(newSceneName));
                        if (newSceneName.StartsWith("Assets/Scenes/Lobby"))   //when going backn from carrom to lobby         
                        {
                            OnSceneReload();  //this method is my modification,look bellow              //to destroy current plugin controller and netwrok manager
                           
                        }
                      
                        //loadingSceneAsync = SceneManager.LoadSceneAsync(newSceneName);       //original statement
                        break;
                    }
        ...
        IEnumerator LoadAsynchronously(string newSceneName)
        {
            if (newSceneName.StartsWith("Assets/Scenes/Lobby"))
            {
                if (pluginContoller.transform.Find("Canvas/Panel_Settings").gameObject.activeSelf)
                {
                    pluginContoller.transform.Find("Canvas/Panel_Settings").gameObject.SetActive(false);
                    pluginContoller.transform.Find("Canvas/BTN_Close_Settings").gameObject.SetActive(false);
                }
                else
                    pluginContoller.transform.Find("Canvas/BTN_Settings").gameObject.SetActive(false);
            ...
            else
            {
                ...
                pluginContoller.transform.Find("Canvas/Panel_CountDown").gameObject.SetActive(true);
                yield return new WaitForSeconds(3f);
                pluginContoller.transform.Find("Canvas/Panel_LoadinScreen").gameObject.SetActive(true) ;
                pluginContoller.transform.Find("Canvas/Panel_CountDown").gameObject.SetActive(false);
                loadingSceneAsync = SceneManager.LoadSceneAsync(newSceneName);
        ...
        public virtual void OnSceneReload() { }       //MY Modification
