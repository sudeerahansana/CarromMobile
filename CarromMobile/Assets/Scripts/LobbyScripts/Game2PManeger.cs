...
public class Game2PManeger : NetworkBehaviour
{
  ...
  //  private bool execute = false;
    ...
    {                                                                               //find previous part above
        ...
        Reda.EventFall += PieceFall;
        Reda.FinishTurn += TurnEnds;
        PlayerSpawSystem.OnPlayerSpawn += ParentDisk;
        Pieces.RespawnDisplacedPieces += ReplacePiece;
        Reda.RespawnDisplacedPieces += ReplacePiece;
    ...
    {
        Debug.Log("Instances Count" + piecesInstances.Count);
    ...
    private void PieceFall(GameObject passedGameObject)
    {
        ...
        else if (index == 1 && pieceNumber < 10)
        {
            if (diskFall)
            {
